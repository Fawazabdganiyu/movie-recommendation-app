...
import { getUserService, getTmdbService } from '../container';
...
export const filterMovies = async (
  ...
  try {
    ...
    const movies = await tmdbService.filterMovies({
      genre,
      minRating,
      maxRating,
      releaseDateGte,
...
export const getRecommendations = async (
  ...
  try {
    ...
    if (userId) {
      try {
        const user = await getUserService().getById(new Types.ObjectId(userId));
        if (user?.preferences) {
